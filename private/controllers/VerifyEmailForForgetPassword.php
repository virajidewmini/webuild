<?php
    ...
    class VerifyEmailForForgetPassword extends Controller{
                ...
                if (isset($_SESSION['otp']) && isset($_SESSION['time'])) {
                    ...
                    // $userEmail = $_SESSION['user_email'];
                        ...
                        if (($timestamp - $_SESSION['time']) > $remainingTime) {  // 60 seconds for 1 minute
                            ...
                            $this->view('verifyEmailForForgetPassword', $data);
                            ...
                            if (empty($data['otp_err'])) {
                                ...
                                if ($data['otp'] == $oldOtp) {
                                    // if($this->userModel->verifyemailCustomer($userId) && $this->userModel->verifyemailUsers($userId) ){
                                        echo '<script>';
                                        echo 'setTimeout(function() { alert("OTP is correct!"); redirectToUpdatePassword(); }, 100);'; // Delayed alert
                                        echo 'function redirectToUpdatePassword() {';
                                        echo '    window.location.href = "' . ROOT . '/Forgetpassword/updatePassword"; ';  
                                        echo '}';
                                        echo '</script>';
                                        exit;
